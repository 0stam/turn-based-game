...
func get_graphic(name : String): # Function for loading with "graphics" variable
	if not name in graphics:
		graphics[name] = load("res://art/".plus_file(name))
	return graphics[name]


func update_board(): # Sets all board's fields graphics to proper values
...
func on_entity_added(index : int): # Adds new row to the entity panel
